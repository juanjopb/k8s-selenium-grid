...
docker run -it --network="host" --env "SELENIUM_REMOTE_HOST=${selenium_remote_host}" --env "SELENIUM_REMOTE_PORT=${selenium_remote_port}" --env "IBMCLOUD_USER=${ibmcloud_username}" --env "IBMCLOUD_PASSWORD=${ibmcloud_password}" nightwatch
sleep 30
